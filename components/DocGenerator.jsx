...
import { generateDocContent } from '../services/gemini';

const DocGenerator = ({ repoName }) => {
  ...
  const handleGenerate = async (docId) => {
    ...
    const content = await generateDocContent(doc.filename, repoName);

    setDocs((prev) =>
      prev.map((d) =>
        d.id === docId
          ? { ...d, status: 'complete', content }
          : d
      )
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {docs.map((doc) => (
          ...
          >
            {doc.status === 'loading' ? (
              <Loader2
                className="animate-spin text-[#3B82F6] mb-3"
                size={28}
              />
            ) : doc.status === 'complete' ? (
              <Check
                className="text-[#10B981] mb-3"
                size={28}
              />
            ) : (
              <FileText
